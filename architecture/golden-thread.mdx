...
The `entanglement_matrix` computes the Gram matrix G = S · S† of all signals. When Signal 3 is added, it changes the eigenvectors of G. This **instantly alters the projection** of Signal 1 and Signal 2 during the `fuse()` step, even though the signals themselves didn't change.
